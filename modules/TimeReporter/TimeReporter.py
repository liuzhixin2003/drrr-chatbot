...
class Game:
    def __init__(self, stage, bot, config_mgr):
        self.bot = bot
        self.conn = list(config_mgr.get_conns().keys())[0]
        self.stage = stage
        self.players = []
        self.n = 0
        self.deck = None
    ...
    def say(self, cont):
        self.bot.send(self.conn, cont)

    def me(self, cont):
        self.say("/me" + cont)

    def dm(self, uid, cont):
        self.bot.dm(self.conn, uid, cont)
...
class TimeReporter(Module):
    ...
    # set of connection names which have active time loops
    ...
    def stop_repeating_task(self, conn_name):
        ...
        for task in asyncio.all_tasks(loop=self.get_event_loop(TimeReporter.KEY_EVT_LOOP)):
    ...
    async def _broadcast_time_loop(self, conn_name):
        ...
        while conn_name in self.repeating_tasks:
            ...
            await asyncio.sleep(self.conf.as_float(TimeReporter.CONF_REPORT_INTERVAL_KEY),
                                loop=self.get_event_loop(TimeReporter.KEY_EVT_LOOP))
    ...
    def start_loop(self, conn_name):
        if conn_name in self.repeating_tasks:
            ...
            asyncio.run_coroutine_threadsafe(self._broadcast_time_loop(conn_name),
                                             self.get_event_loop(TimeReporter.KEY_EVT_LOOP))
    ...
    def handler(self, conn_name, message):
        ...
        if type == tmsg or type == tme or type == tdm:
            ...
            def say(cont):
                self.bot.send(conn_name, cont)

            def me(cont):
                say("/me" + cont)

            def dm(uid, cont):
                self.bot.dm(conn_name, uid, cont)
...
'''
    ...
    def __init__(self, config_mgr, perms_mgr, bot):
        ...
        self.game = Game(Stage(), bot, config_mgr)
        # interval in seconds
