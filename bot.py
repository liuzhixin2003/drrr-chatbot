...
### sending ends here
    ...
    def enter(self, conn_name, cookie_str):
        cookies = {}
        ...
        cookie_path = os.path.join(os.path.abspath(
                self.config_mgr.cookies_dir()), 
                self.conn[conn_name].id + ".cookie")
        self.logger.debug(cookie_path)
        cookie_jar.save(cookie_path)
        self.logger.debug("successfully logged in with cookies!")
        self.resume(conn_name)
...
class BotCLI(Cmd):
    ...
    def do_enter(self, args):
        ...
        if len(arg_split) != 2:
            print("provide a connection name and cookie")
        else:
            self.bot.enter(arg_split[0], arg_split[1])
